fn main() {
    ...
    let mut resolver = statico::resolution::Resolver::new(root);
    ...
    let mut workspace_aliases: Vec<(String, Vec<oxc_resolver::AliasValue>)> = Vec::new();
    for entry in walkdir::WalkDir::new(root).max_depth(5).into_iter().filter_map(|e| e.ok()) {
        let path = entry.path();
        if !path.is_file() || path.file_name().map_or(true, |n| n != "package.json") { continue; }
        ...
        let name = match pkg.get("name").and_then(|v| v.as_str()) { Some(n) => n, None => continue };
        if !name.starts_with('@') && !name.contains('/') { continue; }
        let pkg_dir = path.parent().unwrap();
        workspace_aliases.push((format!("{}$", name), vec![oxc_resolver::AliasValue::Path(pkg_dir.to_string_lossy().to_string())]));
        workspace_aliases.push((name.to_string(), vec![oxc_resolver::AliasValue::Path(pkg_dir.to_string_lossy().to_string())]));
    }
    println!("  ({} workspace aliases)", workspace_aliases.len());
    let options2 = oxc_resolver::ResolveOptions {
        extensions: vec![".ts".into(), ".tsx".into(), ".js".into(), ".jsx".into(), ".mjs".into(), ".cjs".into(), ".d.ts".into()],
        main_fields: vec!["module".into(), "main".into(), "types".into(), "typings".into()],
        main_files: vec!["index".into()],
        condition_names: vec!["import".into(), "module".into(), "require".into(), "default".into(), "types".into(), "node".into()],
        tsconfig: None,
    ...
    for (file, spec) in &test_cases {
        ...
        println!("  {} from {}", spec, result.map(|r| r.full_path().to_string_lossy().to_string()).unwrap_or_else(|e| format!("ERR: {}", e)));
    }
}
