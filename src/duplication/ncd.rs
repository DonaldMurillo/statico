...
pub fn find_candidate_pairs(file_sources: &[(String, String)], threshold: f64, min_lines: usize) -> Vec<NcdCandidate> {
    ...
    for i in 0..eligible.len() {
        for j in (i + 1)..eligible.len() {
            ...
            let (min_c, max_c) =
                if c_sizes[i] < c_sizes[j] { (c_sizes[i], c_sizes[j]) } else { (c_sizes[j], c_sizes[i]) };
...
pub fn detect_ncd_duplicates(file_sources: &[(String, String)]) -> Vec<DuplicateCodeIssue> {
    ...
    let line_counts: HashMap<&str, usize> = file_sources.iter().map(|(p, s)| (p.as_str(), s.lines().count())).collect();
...
mod tests {
    ...
    fn find_candidates_filters_by_threshold() {
        ...
        assert!(
            candidates.iter().any(|c| c.path_a == "a.rs" && c.path_b == "b.rs"),
            "identical pair should be flagged"
        );
        assert!(
            !candidates.iter().any(|c| (c.path_a == "a.rs" || c.path_a == "b.rs") && c.path_b == "c.rs"),
