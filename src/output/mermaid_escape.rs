...
mod tests {
    ...
    fn sec_mermaid_display_name_escapes_hash() {
        ...
        assert!(name.contains("&#35;"), "# should become &#35;, got: {}", name);
        let without_entities = name.replace("&amp;", "").replace("&#35;", "").replace("&quot;", "");
        assert!(!without_entities.contains('#'), "no raw # should remain, got: {}", name);
    }
    ...
    fn sec_mermaid_display_name_escapes_ampersand() {
        ...
        let raw_amp = name
            ...
            .replace("&#123;", "")
            .replace("&#125;", "");
        assert!(!raw_amp.contains('&'), "no unescaped & should remain, got: {}", name);
    }

    #[test]
    fn sec_mermaid_label_escapes_ampersand() {
        let escaped = escape_mermaid_label("foo&amp;evil");
        assert!(escaped.contains("&amp;"), "& should become &amp;, got: {:?}", escaped);
        assert!(escaped.contains("&amp;amp;"), "nested & should be double-escaped, got: {:?}", escaped);
    }
}
